...
import { useKoopayInitializeEscrow } from "./useKoopayInitializeEscrow";
...
interface KoopayCreateProjectFormProps {
  onSuccess?: (
    escrow: InitializeMultiReleaseEscrowPayload & { contractId: string }
  ) => void;
}

export const KoopayCreateProjectForm: React.FC<KoopayCreateProjectFormProps> = ({
  onSuccess,
}) => {
  const {
    form,
  ...
  } = useKoopayInitializeEscrow({ onSuccess });
        ...
        <Card className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 p-4">
          ...
          >
            ...
            <p className="text-muted-foreground mt-1">
              This will create a multi-release escrow contract on the Stellar
              network.
            </p>
          </Link>
        </Card>
