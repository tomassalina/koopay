  ...
  return (
    ...
    >
      ...
      <div className="flex items-start gap-16 relative z-10">
            ...
            <div className="relative w-full h-full" style={{ isolation: 'isolate' }}>
              ...
              <div 
                className={`absolute inset-0 rounded-full cursor-pointer transition-all duration-300 ease-out ${
                  hoveredSegment === 0 ? 'scale-110 translate-x-1 translate-y-1' : 'scale-100 translate-x-0 translate-y-0'
                }`}
                style={{
              ...
              <div 
                className={`absolute inset-0 rounded-full cursor-pointer transition-all duration-300 ease-out ${
                  hoveredSegment === 1 ? 'scale-110 -translate-x-1 translate-y-1' : 'scale-100 translate-x-0 translate-y-0'
                }`}
                style={{
                  background: `conic-gradient(
              ...
              <div 
                className={`absolute inset-0 rounded-full cursor-pointer transition-all duration-300 ease-out ${
                  hoveredSegment === 2 ? 'scale-110 -translate-x-1 -translate-y-1' : 'scale-100 translate-x-0 translate-y-0'
                }`}
                style={{
                  background: `conic-gradient(
            ...
            <div className="text-center">
              {hoveredSegment !== null ? (
                <div className="flex flex-col items-center space-y-2">
                  <div className="text-white/80">{data[hoveredSegment].icon}</div>
                  <div className="text-2xl font-bold text-white">{data[hoveredSegment].value}%</div>
                  <div className="text-sm text-white/60">{data[hoveredSegment].label}</div>
                </div>
              ) : (
        ...
        <div className="flex flex-col justify-between h-48">
          ...
          <div className="space-y-2">
            ...
            <div className="text-3xl font-bold text-white">${totalBalance.toLocaleString('en-US')}</div>
          </div>
      ...
      <div className="mt-8 flex items-center space-x-2 relative z-10">
        ...
        <span className="text-white/80">{completedProjects} proyectos realizados</span>
      </div>
    </div>
  )
}
